...
- Registration: `Registry` grants a 30-second lease and renews it every nine seconds. A terminal keepalive failure flags that lease for the recovery worker, which grants a fresh lease and republishes the last successfully registered key/value. Each etcd operation has a two-second timeout; unsuccessful recovery retries after one second. Shutdown serializes with publication, stops and joins the worker, then revokes only its owned lease, so an old instance cannot delete a newer instance's registration. Initial registration failure retains its startup failure behavior. RL-DISCOVERY-02 expires the lease and requires registration and account RPC to recover in the same Identity and caller processes.
