...
**Flow:** Client WebSocket `MSG_PUSH_ACK` -> Push connection and stored-payload validation -> asynchronous Message `UpdateReadAck` -> MySQL convergence -> conditional Redis unacked removal.

Target: `3.0-dev`. Updated: 2026-09-14. Status: Current in the Issue #58 candidate; release integration remains pending.
...
- Trust: Push binds identity when `CLIENT_AUTH` verifies the JWT, then compares ACK body `user_id` and `device_id` with connection identity. It reads the exact per-device `user_seq` payload from Redis, verifies the delivered message ID and user sequence, and checks any supplied conversation/sequence against that payload. Omitted optional watermarks are derived from the stored message. Control notifications cannot advance a conversation cursor. The WS handler constructs server-side brpc auth metadata for Message. Internal caller authentication remains Issue #61; the existing authenticated HTTP `update_read_ack` route is unchanged and this WS validation does not secure arbitrary direct internal RPC callers.
...
- Async/retry: A Message RPC has a three-second timeout and no automatic RPC retry. Missing channels, Redis errors, transport failure, and `header.success=false` retain the existing unacked delivery. The callback owns its channel and Redis client until completion. Only successful durable convergence permits a same-slot Lua compare-and-delete of the exact stored payload; a replacement payload survives an older callback. A crash before removal permits replay; a crash after the database commit only repeats an idempotent update. Heartbeats resend pending deliveries and clients ACK them again, including after reconnecting to a restarted Push. Recovery still requires client participation within the existing unacked TTL; this change does not add a durable ACK-intent worker or extend Redis durability/retention guarantees.
- Transaction: `ConversationMemberTable::update_last_ack_seq` locks an active member row and advances `last_ack_seq` monotonically within that transaction. Membership departure cannot slip between validation and the cursor write. ODB binds the identity parameters; no Message-row lookup is added. Missing membership returns 3002, while a failed transaction returns retryable 9002. `last_read_seq` remains independent.
- Tests: `tests/func/delivery_ack_test.go` covers forged fields (FN-WS-10), optional/duplicate/ordered ACK compatibility (FN-WS-11), business failures (FN-WS-12), and membership races (FN-MS-02). `tests/reliability/delivery_ack_test.go` (RL-ACK-01) covers Message outage, Push restart, replay and convergence. `tests/perf/delivery_ack_test.go` (PF-10) samples durable ACK latency and SQL work. Existing direct `UpdateReadAck` and unread Scenario tests remain required regressions.
