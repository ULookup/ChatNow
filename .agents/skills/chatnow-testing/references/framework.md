...
The Reliability target runs the complete tagged package and does not consume `TEST_RUN`. For an exact test, invoke the same tagged Go package with an anchored `-run` expression, then run `make -C tests test-reliability` for the layer regression. Current controllers cover Redis, the scoped Identity endpoint move, and Message stop/start; there is no RabbitMQ, MySQL, arbitrary-service, or general-network controller.
