...
public:
    ...
    void getMyselfAsync();
    UserInfo* getMyself();
    void resetMyself(std::shared_ptr<chatnow::GetUserInfoRsp> resp);

    // 获取好友列表
    void getFriendListAsync();
    QList<UserInfo>* getFriendList();
    void resetFriendList(std::shared_ptr<chatnow::GetFriendListRsp> resp);

    // 获取会话列表
    void getChatSessionListAsync();
    QList<ChatSessionInfo>* getChatSessionList();
    void resetChatSessionList(std::shared_ptr<chatnow::GetChatSessionListRsp> resp);

    // 获取好友申请列表
    void getApplyListAsync();
    QList<UserInfo>* getApplyList();
    void resetApplyList(std::shared_ptr<chatnow::GetPendingFriendEventListRsp> resp);
    ...
    void resetRecentMessageList(const QString& chatSessionId, std::shared_ptr<chatnow::GetRecentMsgRsp> resp);
    ...
    // 获取短信验证码
    void getVerifyCodeAsync(const QString& mail);
    void resetVerifyCodeId(const QString& verifyCodeId);
    // 获取 verifyCodeId
    const QString& getVerifyCodeId();

    // 修改手机号码
    void changeMailAsync(const QString& mail, const QString& verifyCodeId, const QString& verifyCode);
    void resetMail(const QString& mail);
    ...
    void resetMemberList(const QString& chatSessionId, const QList<chatnow::UserInfo>& memberList);

    // 搜索用户
    void searchUserAsync(const QString& searchKey);
    QList<UserInfo>* getSearchUserResult();
    void resetSearchUserResult(const QList<chatnow::UserInfo>& userList);
    ...
    void resetSearchMessageResult(const QList<chatnow::MessageInfo>& msgList);
    ...
    void mailLoginAsync(const QString& mail, const QString& verifyCode);
    void mailRegisterAsync(const QString& mail, const QString& verifyCode);
