...
void DataCenter::resetMyself(std::shared_ptr<chatnow::GetUserInfoRsp> resp)
{
    if (myself == nullptr) {
        myself = new UserInfo();
    }
    const chatnow::UserInfo& userInfo = resp->userInfo();
...
void DataCenter::resetFriendList(std::shared_ptr<chatnow::GetFriendListRsp> resp)
{
    ...
    QList<chatnow::UserInfo>& friendListPB = resp->friendList();
...
void DataCenter::resetChatSessionList(std::shared_ptr<chatnow::GetChatSessionListRsp> resp)
{
...
void DataCenter::resetApplyList(std::shared_ptr<chatnow::GetPendingFriendEventListRsp> resp)
{
...
void DataCenter::resetRecentMessageList(const QString &chatSessionId, std::shared_ptr<chatnow::GetRecentMsgRsp> resp)
{
...
void DataCenter::getVerifyCodeAsync(const QString &mail)
{
    // 这个操作, 不需要传入 loginSessionId
    // 后续还需要实现通过手机验证码登录. 登录的时候, 没有 loginSessionId 的
    netClient.getVerifyCode(mail);
}
...
void DataCenter::changeMailAsync(const QString &mail, const QString &verifyCodeId, const QString &verifyCode)
{
    netClient.changeMail(loginSessionId, mail, verifyCodeId, verifyCode);
}

void DataCenter::resetMail(const QString &mail)
{
    if (myself == nullptr) {
        return;
    }
...
void DataCenter::resetMemberList(const QString &chatSessionId, const QList<chatnow::UserInfo> &memberList)
{
...
void DataCenter::resetSearchUserResult(const QList<chatnow::UserInfo> &userList)
{
...
void DataCenter::resetSearchMessageResult(const QList<chatnow::MessageInfo> &msgList)
{
...
void DataCenter::mailLoginAsync(const QString &mail, const QString &verifyCode)
{
    netClient.mailLogin(mail, this->currentVerifyCodeId, verifyCode);
}

void DataCenter::mailRegisterAsync(const QString &mail, const QString &verifyCode)
{
    netClient.mailRegister(mail, this->currentVerifyCodeId, verifyCode);
}
