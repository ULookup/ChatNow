...
void NetClient::initWebsocket()
{
    ...
    connect(&websocketClient, &QWebSocket::binaryMessageReceived, this, [=](const QByteArray& byteArray) {
        ...
        chatnow::NotifyMessage notifyMessage;
...
void NetClient::handleWsResponse(const chatnow::NotifyMessage &notifyMessage)
{
    if (notifyMessage.notifyType() == chatnow::NotifyTypeGadget::NotifyType::CHAT_MESSAGE_NOTIFY) {
        // 收到消息
    ...
    } else if (notifyMessage.notifyType() == chatnow::NotifyTypeGadget::NotifyType::CHAT_SESSION_CREATE_NOTIFY) {
        // 创建新的会话通知
        ChatSessionInfo chatSessionInfo;
        chatSessionInfo.load(notifyMessage.newChatSessionInfo().chatSessionInfo());
        handleWsSessionCreate(chatSessionInfo);
    } else if (notifyMessage.notifyType() == chatnow::NotifyTypeGadget::NotifyType::FRIEND_ADD_APPLY_NOTIFY) {
        // 添加好友申请通知
        UserInfo userInfo;
        userInfo.load(notifyMessage.friendAddApply().userInfo());
        handleWsAddFriendApply(userInfo);
    } else if (notifyMessage.notifyType() == chatnow::NotifyTypeGadget::NotifyType::FRIEND_ADD_PROCESS_NOTIFY) {
        // 添加好友申请的处理结果通知
        UserInfo userInfo;
    ...
    } else if (notifyMessage.notifyType() == chatnow::NotifyTypeGadget::NotifyType::FRIEND_REMOVE_NOTIFY) {
        // 删除好友通知
...
{
    chatnow::ClientAuthenticationReq req;
...
void NetClient::getMyself(const QString &loginSessionId)
{
    ...
    connect(httpResp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto resp = handleHttpResponse<chatnow::GetUserInfoRsp>(httpResp, &ok, &reason);
...
void NetClient::getFriendList(const QString& loginSessionId)
{
    ...
    connect(httpResp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto friendListResp = this->handleHttpResponse<chatnow::GetFriendListRsp>(httpResp, &ok, &reason);
...
{
    // 1. 通过 protobuf 构造 body
    chatnow::GetChatSessionListReq req;
    req.setRequestId(makeRequestId());
    req.setSessionId(loginSessionId);
    QByteArray body = req.serialize(&serializer);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::GetChatSessionListRsp>(resp, &ok, &reason);
...
{
    // 1. 通过 protobuf 构造 body
    chatnow::GetPendingFriendEventListReq req;
    req.setRequestId(makeRequestId());
    req.setSessionId(loginSessionId);
    QByteArray body = req.serialize(&serializer);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::GetPendingFriendEventListRsp>(resp, &ok, &reason);
...
void NetClient::getRecentMessageList(const QString &loginSessionId, const QString &chatSessionId, bool updateUI)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::GetRecentMsgRsp>(resp, &ok, &reason);
                            ...
                            const QByteArray &content, const QString& extraInfo)
{
    ...
    if (messageType == MessageType::TEXT_TYPE) {
        messageContent.setMessageType(chatnow::MessageTypeGadget::MessageType::STRING);

        chatnow::StringMessageInfo stringMessageInfo;
        ...
        messageContent.setMessageType(chatnow::MessageTypeGadget::MessageType::IMAGE);

        chatnow::ImageMessageInfo imageMessageInfo;
        ...
        messageContent.setMessageType(chatnow::MessageTypeGadget::MessageType::FILE);

        chatnow::FileMessageInfo fileMessageInfo;
        ...
        messageContent.setMessageType(chatnow::MessageTypeGadget::MessageType::SPEECH);

        chatnow::SpeechMessageInfo speechMessageInfo;
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::NewMessageRsp>(resp, &ok, &reason);
...
{
    // 1. 通过 protobuf 构造请求 body
    chatnow::SetUserNicknameReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::SetUserNicknameRsp>(resp, &ok, &reason);
...
{
    // 1. 通过 protobuf 构造请求 body
    chatnow::SetUserDescriptionReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::SetUserDescriptionRsp>(resp, &ok, &reason);
...
void NetClient::getVerifyCode(const QString &mail)
{
    // 1. 构造请求 body
    chatnow::MailVerifyCodeReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setMailNumber(mail);
    QByteArray body = pbReq.serialize(&serializer);
    LOG() << "[获取手机验证码] 发送请求 requestId=" << pbReq.requestId() << ", mail=" << mail;

    // 2. 发送 HTTP 请求
    QNetworkReply* resp = this->sendHttpRequest("/service/user/get_mail_verify_code", body);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::MailVerifyCodeRsp>(resp, &ok, &reason);
...
void NetClient::changeMail(const QString &loginSessionId, const QString &mail, const QString &verifyCodeId, const QString &verifyCode)
{
    // 1. 构造请求 body
    chatnow::SetUserMailNumberReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    pbReq.setMailNumber(mail);
    pbReq.setMailVerifyCodeId(verifyCodeId);
    pbReq.setMailVerifyCode(verifyCode);
    QByteArray body = pbReq.serialize(&serializer);
    LOG() << "[修改手机号] 发送请求 requestId=" << pbReq.requestId() << ", loginSessionId=" << pbReq.sessionId()
          << ", mail=" << pbReq.mailNumber() << ", verifyCodeId=" << pbReq.mailVerifyCodeId() << ", verifyCode=" << pbReq.mailVerifyCode();

    // 2. 发送 http 请求
    QNetworkReply* resp = this->sendHttpRequest("/service/user/set_mail", body);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::SetUserMailNumberRsp>(resp, &ok, &reason);
        ...
        dataCenter->resetMail(mail);

        // d) 发送信号, 通知调用者完成
        emit dataCenter->changeMailDone();
...
{
    // 1. 构造请求 body
    chatnow::SetUserAvatarReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::SetUserAvatarRsp>(resp, &ok, &reason);
...
void NetClient::deleteFriend(const QString &loginSessionId, const QString &userId)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::FriendRemoveRsp>(resp, &ok, &reason);
...
void NetClient::addFriendApply(const QString &loginSessionId, const QString &userId)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::FriendAddRsp>(resp, &ok, &reason);
...
{
    // 1. 构造请求 body
    chatnow::FriendAddProcessReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    pbReq.setAgree(true);
...
{
    // 1. 构造请求 body
    chatnow::ChatSessionCreateReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::ChatSessionCreateRsp>(resp, &ok, &reason);
...
{
    // 1. 构造请求 body
    chatnow::GetChatSessionMemberReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    pbReq.setChatSessionId(chatSessionId);
    QByteArray body = pbReq.serialize(&serializer);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::GetChatSessionMemberRsp>(resp, &ok, &reason);
...
void NetClient::searchUser(const QString &loginSessionId, const QString &searchKey)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::FriendSearchRsp>(resp, &ok, &reason);
...
void NetClient::searchMessage(const QString &loginSessionId, const QString &chatSessionId, const QString &searchKey)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::MsgSearchRsp>(resp, &ok, &reason);
...
{
    // 1. 构造请求 body
    chatnow::GetHistoryMsgReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    pbReq.setChatSessionId(chatSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::GetHistoryMsgRsp>(resp, &ok, &reason);
...
void NetClient::userLogin(const QString &username, const QString &password)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::UserLoginRsp>(resp, &ok, &reason);
...
void NetClient::userRegister(const QString &username, const QString &password)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::UserRegisterRsp>(resp, &ok, &reason);
...
void NetClient::mailLogin(const QString &mail, const QString &verifyCodeId, const QString &verifyCode)
{
    // 1. 构造请求 body
    chatnow::MailLoginReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setMailNumber(mail);
    pbReq.setVerifyCodeId(verifyCodeId);
    pbReq.setVerifyCode(verifyCode);
    QByteArray body = pbReq.serialize(&serializer);
    LOG() << "[手机号登录] 发送请求 requestId=" << pbReq.requestId() << ", mail=" << pbReq.mailNumber()
          << ", verifyCodeId=" << pbReq.verifyCodeId() << ", verifyCode=" << pbReq.verifyCode();

    // 2. 发送 HTTP 请求
    QNetworkReply* resp = this->sendHttpRequest("/service/user/mail_login", body);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::MailLoginRsp>(resp, &ok, &reason);

        // b) 判定响应是否成功
        if (!ok) {
            LOG() << "[手机号登录] 响应出错! reason=" << reason;
            emit dataCenter->mailLoginDone(false, reason);
            return;
        }
        ...
        // d) 发送信号
        emit dataCenter->mailLoginDone(true, "");
...
void NetClient::mailRegister(const QString& mail, const QString& verifyCodeId, const QString& verifyCode) {
    // 1. 构造请求 body
    chatnow::MailRegisterReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setMailNumber(mail);
    pbReq.setVerifyCodeId(verifyCodeId);
    pbReq.setVerifyCode(verifyCode);
    QByteArray body = pbReq.serialize(&serializer);
    LOG() << "[手机号注册] 发送请求 requestId=" << pbReq.requestId() << ", mail=" << pbReq.mailNumber()
          << ", verifyCodeId=" << pbReq.verifyCodeId() << ", verifyCode=" << pbReq.verifyCode();

    // 2. 发送 HTTP 请求
    QNetworkReply* resp = this->sendHttpRequest("/service/user/mail_register", body);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::MailRegisterRsp>(resp, &ok, &reason);

        // b) 判定响应是否成功
        if (!ok) {
            LOG() << "[手机号注册] 响应失败! reason=" << reason;
            emit dataCenter->mailRegisterDone(false, reason);
            return;
        }
        ...
        // d) 发送信号
        emit dataCenter->mailRegisterDone(true, "");
...
{
    // 1. 构造请求 body
    chatnow::GetSingleFileReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::GetSingleFileRsp>(resp, &ok, &reason);
...
{
    // 1. 构造请求 body
    chatnow::SpeechRecognitionReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<chatnow::SpeechRecognitionRsp>(resp, &ok, &reason);
