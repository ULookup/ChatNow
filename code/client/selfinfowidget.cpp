...
SelfInfoWidget::SelfInfoWidget(QWidget* parent) : QDialog(parent) {
    ...
    // 7. 添加电话
    mailTag = new QLabel();
    mailTag->setFixedSize(50, height);
    mailTag->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    mailTag->setText("电话");
    mailTag->setStyleSheet(labelStyle);

    mailLabel = new QLabel();
    mailLabel->setFixedHeight(height);
    mailLabel->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);

    mailModifyBtn = new QPushButton();
    mailModifyBtn->setFixedSize(70, 25);
    mailModifyBtn->setIconSize(QSize(20, 20));
    mailModifyBtn->setIcon(QIcon(":/resource/image/modify.png"));
    mailModifyBtn->setStyleSheet(btnStyle);

    mailEdit = new QLineEdit();
    mailEdit->setFixedHeight(height);
    mailEdit->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
    mailEdit->setStyleSheet(editStyle);
    mailEdit->hide();

    mailSubmitBtn = new QPushButton();
    mailSubmitBtn->setFixedSize(70, 25);
    mailSubmitBtn->setIconSize(QSize(20, 20));
    mailSubmitBtn->setIcon(QIcon(":/resource/image/submit.png"));
    mailSubmitBtn->setStyleSheet(btnStyle);
    mailSubmitBtn->hide();
    ...
    layout->addWidget(mailTag, 3, 1);
    layout->addWidget(mailLabel, 3, 2);
    layout->addWidget(mailModifyBtn, 3, 3);

#if TEST_UI
    idLabel->setText("1234");
    nameLabel->setText("张三");
    descLabel->setText("从今天开始认真敲代码");
    mailLabel->setText("18612345678");
...
void SelfInfoWidget::initSingalSlot()
{
    ...
    connect(mailModifyBtn, &QPushButton::clicked, this, [=]() {
        mailLabel->hide();
        mailModifyBtn->hide();
        layout->removeWidget(mailLabel);
        layout->removeWidget(mailModifyBtn);

        mailEdit->show();
        mailSubmitBtn->show();
        layout->addWidget(mailEdit, 3, 2);
        layout->addWidget(mailSubmitBtn, 3, 3);
        ...
        mailEdit->setText(mailLabel->text());
    });
    ...
    connect(mailSubmitBtn, &QPushButton::clicked, this, &SelfInfoWidget::clickMailSubmitBtn);
...
{
    // 1. 获取到输入框中的手机号码
    const QString& mail = mailEdit->text();
    if (mail.isEmpty()) {
        return;
    }
    ...
    dataCenter->getVerifyCodeAsync(mail);

    // 3. 把刚才发送请求的手机号码, 保存起来.
    //    后续点击提交按钮, 修改电话, 修改的号码, 不从输入框读取, 而是读取这个变量.
    this->mailToChange = mail;
...
void SelfInfoWidget::clickMailSubmitBtn()
{
    // 1. 先判定, 当前验证码是否已经收到.
    DataCenter* dataCenter = DataCenter::getInstance();
    ...
    connect(dataCenter, &DataCenter::changeMailDone, this, &SelfInfoWidget::clickMailSubmitBtnDone, Qt::UniqueConnection);
    dataCenter->changeMailAsync(this->mailToChange, verifyCodeId, verifyCode);

    // 4. 让验证码按钮的倒计时停止. 把 leftTime 设为 1, 就可以停止了
    leftTime = 1;
}

void SelfInfoWidget::clickMailSubmitBtnDone()
{
    layout->removeWidget(verifyCodeTag);
    layout->removeWidget(verifyCodeEdit);
    layout->removeWidget(getVerifyCodeBtn);
    layout->removeWidget(mailEdit);
    layout->removeWidget(mailSubmitBtn);
    verifyCodeTag->hide();
    verifyCodeEdit->hide();
    getVerifyCodeBtn->hide();
    mailEdit->hide();
    ...
    layout->addWidget(mailLabel, 3, 2);
    mailLabel->show();
    mailLabel->setText(this->mailToChange);
    layout->addWidget(mailModifyBtn, 3, 3);
    mailModifyBtn->show();
}
