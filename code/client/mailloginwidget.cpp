...
MailLoginWidget::MailLoginWidget(QWidget *parent)
    : QWidget{parent}
{
    // 1. 设置窗口的基本属性
    ...
    mailEdit->setPlaceholderText("输入手机号");
    mailEdit->setFixedHeight(40);
    ...
    layout->addWidget(mailEdit, 1, 0, 1, 5);
    ...
    // 11. 添加信号槽
    connect(switchModeBtn, &QPushButton::clicked, this, &MailLoginWidget::switchMode);
    ...
    connect(sendVerifyCodeBtn, &QPushButton::clicked, this, &MailLoginWidget::sendVerifyCode);

    timer = new QTimer(this);
    connect(timer, &QTimer::timeout, this, &MailLoginWidget::countDown);

    connect(submitBtn, &QPushButton::clicked, this, &MailLoginWidget::clickSubmitBtn);
}

void MailLoginWidget::sendVerifyCode()
{
    // 1. 获取到手机号码
    const QString& mail = this->mailEdit->text();
    if (mail.isEmpty()) {
        return;
    }
    ...
    connect(dataCenter, &DataCenter::getVerifyCodeDone, this, &MailLoginWidget::sendVerifyCodeDone, Qt::UniqueConnection);
...
void MailLoginWidget::sendVerifyCodeDone()
{
    Toast::showMessage("验证码已经发送");
}

void MailLoginWidget::clickSubmitBtn()
{
    // 1. 从输入框中拿到必要的内容
    //    此处不要从输入框直接拿电话. 可能用户故意搞事情, 输入验证码的时候, 输入的是电话1, 点击提交的时候, 输入的是电话2;
    const QString& mail = this->currentMail;
    const QString& verifyCode = verifyCodeEdit->text();
    if (mail.isEmpty() || verifyCode.isEmpty()) {
    ...
    if (isLoginMode) {
        // 登录
        connect(dataCenter, &DataCenter::mailLoginDone, this, &MailLoginWidget::mailLoginDone, Qt::UniqueConnection);
        dataCenter->mailLoginAsync(mail, verifyCode);
    } else {
        // 注册
        connect(dataCenter, &DataCenter::mailRegisterDone, this, &MailLoginWidget::mailRegisterDone, Qt::UniqueConnection);
        dataCenter->mailRegisterAsync(mail, verifyCode);
    }
}

void MailLoginWidget::mailLoginDone(bool ok, const QString& reason)
{
    if (!ok) {
...
void MailLoginWidget::mailRegisterDone(bool ok, const QString &reason)
{
    // 是否注册成功
    if (!ok) {
...
void MailLoginWidget::countDown()
{
    if (leftTime <= 1) {
...
void MailLoginWidget::switchMode()
{
    if (isLoginMode) {
        // 切换到注册模式
